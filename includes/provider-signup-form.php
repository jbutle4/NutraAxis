...
?>
<section class="marketing-signup-form-section">
  <div class="marketing-signup-form-section__inner">
    <p class="marketing-signup-stub__eyebrow">Provider Application</p>
    <h2 class="marketing-signup-stub__title">Complete your provider signup</h2>
    <p class="marketing-signup-stub__lead">
      Save a draft at any time. Submit when all required company, compliance, and banking information is complete.
    </p>

    <?php if (!empty($notice)): ?>
    <div class="marketing-signup-alert marketing-signup-alert--success" role="status"><?= htmlspecialchars($notice) ?></div>
    <?php endif; ?>
    <?php if (!empty($warn)): ?>
    <div class="marketing-signup-alert marketing-signup-alert--warn" role="status"><?= htmlspecialchars($warn) ?></div>
    <?php endif; ?>
    <?php if (!empty($error)): ?>
    <div class="marketing-signup-alert marketing-signup-alert--error" role="alert"><?= htmlspecialchars($error) ?></div>
    <?php endif; ?>

    <div class="marketing-signup-meta">
    ...
    <?php if (!$editable): ?>
    <div class="marketing-signup-alert marketing-signup-alert--info" role="status">
      This application has been submitted and is under review. Contact operations if you need changes.
    </div>
    <?php endif; ?>

    <form class="marketing-signup-form" method="post" action="/provider-signup/apply.php?token=<?= rawurlencode($token) ?>" novalidate>
      <input type="hidden" name="access_token" value="<?= htmlspecialchars($token) ?>" />

      <fieldset class="marketing-signup-fieldset" <?= $editable ? '' : 'disabled' ?>>
        <legend>ACCS company information</legend>
        <div class="marketing-signup-grid">
          ...
          <label class="marketing-signup-grid--full">Street address *
      ...
      <?php if ($editable): ?>
      <div class="marketing-signup-form__actions">
        <button class="marketing-button marketing-button--outline" type="submit" name="action" value="save_draft">Save draft</button>
        <button
      ...
      <p class="marketing-signup-checklist">Still needed before submit: <?= htmlspecialchars(implode(', ', $checklist['missing'])) ?></p>
      <?php endif; ?>
      <?php endif; ?>
    </form>

    <form
      class="marketing-signup-upload"
      method="post"
    ...
    >
      ...
      <button class="marketing-button marketing-button--outline" type="submit" name="action" value="upload_certificate">Upload certificate</button>
    </form>

    <?php if ($attachments !== []): ?>
    <div class="marketing-signup-uploaded">
      <p class="marketing-signup-uploaded__title">Uploaded documents</p>
      <ul>
