...
/**
 * Convert a linked NonInventory/Expense QBO item to Inventory @ QtyOnHand=0.
 * Inactivates the prior item and creates a new Inventory twin, then updates SKUMaster.
 */
function catalog_convert_sku_to_qbo_inventory(int $skuId): array
{
    require_once __DIR__ . '/quickbooks.php';
    ...
    $readyError = catalog_validate_qbo_item_ready($sku);
    if ($readyError !== null) {
    ...
    if ($existingId !== '') {
        ...
        if ($fetch['ok'] && is_array($fetch['item'] ?? null)) {
            $existingType = (string) ($fetch['item']['Type'] ?? '');
            if (strcasecmp($existingType, 'Inventory') === 0) {
                // Already inventory — refresh link and ensure sync status.
                catalog_apply_qbo_item_response($skuId, $fetch['item']);

                return [
                    'ok' => true,
                    'error' => null,
                    'action' => 'already_inventory',
                    'item' => $fetch['item'],
                ];
            }

            // Inactivate non-inventory item before creating inventory twin.
            $inactivePayload = $fetch['item'];
            $inactivePayload['Active'] = false;
            $inactivePayload['sparse'] = true;
            $inactiveResult = qbo_create_or_update_item([
                'Id' => $existingId,
                'SyncToken' => (string) ($fetch['item']['SyncToken'] ?? ''),
                'Active' => false,
                'sparse' => true,
            ]);
            if (!$inactiveResult['ok']) {
                return [
                    'ok' => false,
                    'error' => 'Unable to inactivate existing QuickBooks item: '
                        . (string) ($inactiveResult['error'] ?? 'unknown error'),
                ];
            }
        }
    }
    ...
    $sku = catalog_get_sku($skuId);
    if ($sku === null) {
        return ['ok' => false, 'error' => 'SKU not found after clearing QuickBooks link.'];
    }

    // Force Inventory create with QtyOnHand=0.
    $payload = catalog_build_qbo_item_payload($sku, true, 'Inventory');
    unset($payload['Id'], $payload['SyncToken']);

    $create = qbo_create_or_update_item($payload);
    if (!$create['ok']) {
        $error = (string) ($create['error'] ?? 'Unable to create Inventory item in QuickBooks.');
        catalog_mark_qbo_sync($skuId, 'Error', mb_substr($error, 0, 500));

        return ['ok' => false, 'error' => $error];
    }

    $item = $create['data']['Item'] ?? null;
    if (!is_array($item)) {
        return ['ok' => false, 'error' => 'QuickBooks did not return the created Inventory item.'];
    }
    ...
    try {
        ...
        return [
            'ok' => false,
            'error' => 'Inventory item created in QuickBooks but local link failed: ' . $e->getMessage(),
            'item' => $item,
        ];
    }
    ...
    return [
        ...
        'error' => null,
        'action' => 'converted',
        'previous_qbo_item_id' => $existingId !== '' ? $existingId : null,
        'item' => $item,
    ];
}

function catalog_convert_all_skus_to_qbo_inventory(): array
{
    require_once __DIR__ . '/quickbooks.php';

    if (!qbo_is_connected()) {
        return ['ok' => false, 'error' => 'QuickBooks is not connected.', 'total' => 0, 'converted' => 0, 'already' => 0, 'failed' => 0, 'failures' => []];
    }
    ...
    foreach ($rows as $row) {
        ...
        if ($result['ok']) {
            if (($result['action'] ?? '') === 'already_inventory') {
                $summary['already']++;
            } else {
                $summary['converted']++;
            }
    ...
    if ($summary['converted'] === 0 && $summary['already'] === 0 && $summary['failed'] > 0) {
        $summary['ok'] = false;
        $summary['error'] = 'All Inventory conversions failed.';
    }
