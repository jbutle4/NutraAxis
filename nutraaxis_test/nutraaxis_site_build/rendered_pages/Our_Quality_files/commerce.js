...
export const CUSTOMER_SIGNUP_PATH = `${CUSTOMER_PATH}/create`;
...
/**
 * Gets the sign in token from the cookie
 * @returns {string} The sign in token
 */
...
export async function performCommerceGraphQLQuery(query, variables, GET = true, USE_TOKEN = false) {
  const GRAPHQL_ENDPOINT = getConfigValue('commerce-endpoint');

  const headers = {
    'Content-Type': 'application/json',
    Store: getConfigValue('headers.cs.Magento-Store-View-Code'),
    'Magento-Website-Code': getConfigValue('headers.cs.Magento-Website-Code'),
    'Magento-Store-View-Code': getConfigValue('headers.cs.Magento-Store-View-Code'),
    'x-api-mesh-vary': 'magento-customer-group',
  };
